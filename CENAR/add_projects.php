...
// Check if the user is logged in and belongs to the CENAR department
session_start();
if (!isset($_SESSION["loggedin"]) || $_SESSION["department"] !== "CENAR") {
    header("Location: ../login.html"); // Redirect to the login page if not logged in or not from the CENAR department
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "INSERT INTO cenar_projects (title, description, status, back, backname, picture, picturename) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    if (mysqli_stmt_execute($statement)) {
        ...
        header("Location: cenarmodify.php"); // Redirect to the CENAR page or any other desired page
        exit;
    } else {
